...
def training_loop(generator,        # generator network
    ...
    for n_iter in range(init_n_iter, config.max_iters):
        ...
        if args.mask == "box":
            ...
            # D training steps:
            batch_real_mask = torch.cat((batch_real, torch.tile(mask, [config.batch_size, 1, 1, 1])), dim=1)
            batch_filled_mask = torch.cat((batch_complete.detach(), torch.tile(mask, [config.batch_size, 1, 1, 1])), dim=1)
            ...
            batch_gen = torch.cat((batch_gen, torch.tile(mask, [config.batch_size, 1, 1, 1])), dim=1)
